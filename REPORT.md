...
Enforcement is graded rather than binary. Severe flags (`disturbing_imagery`, graphic violence) hard-exclude from retrieval; milder ones (`threat`, `intense_fear`, `death`) only down-weight. Since retrieved stories are Planner inspiration and are never narrated to the child, blanket exclusion would discard legitimate structure — most `quest/rescue` arcs require a threat. Output safety stays with the Judge, which is the only place it reaches the child.
